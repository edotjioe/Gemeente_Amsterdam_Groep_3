...
function(input, output, session) {
  ...
  if (!is.null(map_fact)) {
    ...
    facts <-
      dbGetQuery(con, "SELECT * FROM facts WHERE year = 2017 AND statistics_id = 1")
    locations <- dbGetQuery(con, "SELECT * FROM locations")
    statistics <- dbGetQuery(con, "SELECT * FROM statistics")
    
    buurten_map <-
      geojsonio::geojson_read("datafiles/GEBIED_BUURTEN.json",
                              what = "sp",
                              stringsAsFactor = FALSE)
    locations <-
      locations %>% slice(match(buurten_map$Buurt_code, locations$neighbourhood_code))
    temp <-
      facts[which(facts$year == 2017 & facts$statistics_id == 1),]
    map_fact <-
      data.frame(locations, value = temp[match(locations$locations_id, temp$locations_id), "value"])
  ...
  server <- function(input, output, session) {
    ...
    observeEvent(input$map_shape_click,
                 show_map_graph(input$map_shape_click))
  }
}